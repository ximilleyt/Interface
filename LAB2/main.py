...
class SettingsScreen(ContentFrame):
    def __init__(self):
        ...
        theme_label.setStyleSheet(f"color: {TEXT_LIGHT};")
        self.theme_combo = QComboBox()
        self.theme_combo.addItems(["Light", "Dark"] )
        self.theme_combo.setStyleSheet(f"background: {DARK_BG}; color: {TEXT_LIGHT}; border: 1px solid {BORDER};")
        form.addRow(theme_label, self.theme_combo)
        save_btn = QPushButton("Сохранить")
        save_btn.setStyleSheet(f"background: {BORDER}; color: {TEXT_LIGHT}; border: none; padding: 6px;")
